...
# 画面内の文字は禁止する。
_SONG_BASE_PROMPT = (
    "Create a lively K-pop-style music video scene based on this illustration, "
    ...
    "Pronunciation is critical: when the lyrics contain エーアイ, sing it clearly as "
    "エー・アイ with both mora groups audible; never shorten it to アイ. "
    "She sings exactly these Japanese lyrics, clearly and in rhythm, one phrase "
    "after another: {lyrics} "
    "Backing music: an upbeat, catchy K-pop broadcast-opening tune at about "
    "128 BPM with punchy 808-style sub bass, crisp trap-influenced drums, bright "
    "synth stabs and a big pre-chorus build. "
...
_SONG_EXTENSION_PROMPT = (
    ...
    "Then the music resolves into a bright, satisfying final chord, like the drop "
    "of a K-pop chorus. Visuals: the music video background keeps the same "
    "composition and style, with rhythmic lights and particles building to a "
    "cheerful finish. "
    "Strictly no on-screen text, no letters, no logos, no subtitles, no captions."
)
...
_SONG_OUTRO_PROMPT = (
    ...
    "same upbeat K-pop track continues without vocals and settles toward a clean "
